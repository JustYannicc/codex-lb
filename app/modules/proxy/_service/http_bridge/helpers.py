...
from typing import TYPE_CHECKING, Any, Final, Literal, Mapping, Protocol, TypeVar, cast
    ...
    *,
    ...
    if timeout > 0:
        retained_owner_finished = await _wait_for_http_bridge_retained_owner(
            future, timeout=timeout, scheduler=scheduler_for(service)
        )
...
async def _wait_for_http_bridge_aborted_owner_within_budget(
    ...
    return timeout_seconds > 0 and await _wait_for_http_bridge_aborted_owner(
        future, timeout=timeout_seconds, scheduler=scheduler_for(service)
    )
...
def _http_bridge_should_attempt_local_bootstrap_rebind(
    ...
    *,
    ...
    if previous_response_id is not None:
        return False
    if exc.failure_phase == "owner_forward" and exc.failure_detail == "owner_input_shape_upgrade_required":
        return _sticky_key_from_turn_state_header(headers) is None
    payload = exc.payload
    if not isinstance(payload, dict):
        return False
    error = payload.get("error")
