...
async def test_http_dispatch_avoids_context_database_work_on_repeated_turns(
    ...
    if enabled:
        ...
        assert context_statements and all(statement.startswith("select") for statement in context_statements)
    context_statements.clear()
    response = await async_client.post("/backend-api/codex/responses", headers=headers, json=payload)
    assert response.status_code == 200
    assert "response.completed" in response.text
...
async def test_cache_miss_still_checks_durable_key_ownership(async_client, monkeypatch, cache_miss, marked):
    ...
    if not marked:
        payload.pop("reasoning")
    response = await async_client.post("/backend-api/codex/responses", headers=other_headers, json=payload)
