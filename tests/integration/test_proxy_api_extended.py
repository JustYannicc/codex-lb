...
async def test_openapi_operation_ids_are_unique_and_thread_goal_methods_stable(async_client):
    ...
    assert (
        schema["paths"]["/v1/responses"]["post"]["requestBody"]["content"]["application/json"]["schema"]
        == {"$ref": "#/components/schemas/V1ResponsesRequest"}
    )
    assert (
        schema["paths"]["/backend-api/codex/responses/compact"]["post"]["requestBody"]["content"][
            "application/json"
        ]["schema"]
        == {"$ref": "#/components/schemas/ResponsesCompactRequest"}
    )
