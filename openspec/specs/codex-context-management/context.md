...
The dispatch cache holds at most 4096 positive session bindings and confirmed participants. It removes repeated context writes without storing note bodies. This local integration preserves the deployed contract: an unmarked cache miss queries durable ownership, so restart or eviction cannot bypass a bound session's key fence or lose participation. Ordinary unbound sessions incur a read and remain untracked.
