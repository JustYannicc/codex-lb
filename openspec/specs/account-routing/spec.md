...
- **AND** no qualifying plan-applicable long-window reset transition matches the persisted deadline
...
#### Scenario: Confirmed blocked Team weekly reset permits peer recovery

- **GIVEN** replica A marked a Team account `RATE_LIMITED` with `blocked_at` and a persisted deadline matching that account's secondary weekly window
- **AND** the 30-second minimum floor has elapsed
- **WHEN** replica B observes a real post-block transition from the matching secondary baseline into a new available weekly window
- **AND** the latest secondary sample remains below `100%`
- **AND** the latest primary sample is post-block and below `100%`
- **THEN** replica B may compare-and-set the account to `ACTIVE` before the old persisted deadline
- **AND** a successful transition clears `reset_at` and `blocked_at`
...
- **AND** fresh usage reports availability but no temporal reset in the plan-applicable long window whose baseline matches that deadline
...
- **THEN** the unrelated long-window reset exception does not apply
