...
Quarantine state MUST be bounded and self-recovering: it is in-memory and session-scoped, expires by TTL (a live session that outlives its quarantine window MUST become reusable again), and is cleared when a response completes with the applicable session-identity or exact recovery-generation fence. Quarantine cleanup MUST NOT write account health, alter routing or account selection, or mutate durable bridge ownership. A primary-key clear MUST be fenced by the completing session's identity and the canonical session registry, so a detached predecessor cannot remove a replacement's newer quarantine. A stale-anchor recovery-origin key MUST be fenced by the exact quarantine generation observed when recovery was authorized; an observed absence or generation mismatch MUST leave a raced quarantine active.
