...
When no anchor exists for a delta-only payload because the client supplied none
and durable continuity was abandoned, and the key carries poison evidence,
planning MUST fail closed as `bridge_previous_response_not_found` rather than
dispatching an unanchored new conversation.
...
- **AND** a subsequent delta-only request on that key still receives the durable anchor when that anchor remains available
