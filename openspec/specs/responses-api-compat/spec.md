...
completion MUST capture its quarantine-clear fence before any completion
await, including the pending lock, alias persistence and durable settlement.
A failed pre-settle load or a later successful load MUST NOT replace that
observation: poison evidence armed after capture MUST survive completion.
...
contract defined in "Direct WebSocket replay never mixes numeric response
sequences" and its one-shot replay succeeds.
...
- **WHEN** an identity-authorized response completes with a matching pre-await evidence fence, or the quarantine TTL elapses
- **THEN** the observed quarantine is cleared
- **AND** post-capture first strikes and independently active weaker fences survive cleanup
