...
- Add an opt-in, loopback-only Desktop relay on the existing client-compatible port 8000. Route native usage through a strict LB pool projection and preserve the original caller on other fixed-destination ChatGPT backend requests.
...
Python CLI, a standalone relay, a strict quota endpoint and pool projection, route-level tests, `docs/client-setup.md`, and a linked Desktop setup page. Uses the established HTTP stack. No database migration, default service listener change, app modification, new required base-install setting, or automatic Desktop restart.
