...
#### Scenario: A session just handed to a request is spared

- **GIVEN** a session was handed to a request that has not yet reached submit, so it carries no admission waiter
- **AND** its durable row is missing
- **WHEN** the purge invalidation bump is processed
- **THEN** the durable liveness check (the reuse path renews the row under the bridge lock) and the idle floor keep the session registered and open
- **AND** it is not offered to the durable lookup
...
#### Scenario: A stalled database cannot pin shutdown on the final flush

- **GIVEN** queued bumps and a database that does not respond
- **WHEN** the invalidation poller stops
- **THEN** the final flush is abandoned at its deadline and the poller stops
- **AND** the namespaces stay queued and logged, and the abandoned write is cancelled rather than left running
