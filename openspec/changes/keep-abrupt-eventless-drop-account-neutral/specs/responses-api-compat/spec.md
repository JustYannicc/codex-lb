...
When an HTTP bridge upstream websocket ends without a close frame and zero application-layer response events were observed for the pending requests, the proxy MUST NOT write per-drop account error-health (`record_error`) for that failure. It MUST record the drop into the windowed eventless account failure signal so that repeated eventless drops on the same account within the window still apply the drain penalty. A drop that carries any upstream close frame (including non-clean codes) or that occurs after response events were streamed MUST keep the existing account penalty semantics. The per-bridge retry circuit MUST still record the failure at bridge scope.
