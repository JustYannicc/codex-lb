...
same observation.
Poison provenance MUST fence capture and cleanup only while the poison-specific
deadline is active. A completion during a surviving weaker-only window MUST
capture and clear the observed raw generation; newer weak or poison evidence
armed after capture MUST survive that completion.
