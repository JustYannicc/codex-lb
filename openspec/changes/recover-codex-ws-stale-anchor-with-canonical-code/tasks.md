...
- [ ] 3.3 `openspec validate --specs` remains pending because the executable is unavailable in this environment. Backend unit/integration suites, Ruff, format, changed-file `ty`, and the proxy architecture check have been rerun on the candidate.
...
- [ ] 5.4 Run focused bridge tests, related unit coverage, Ruff/format/type
  checks, and strict OpenSpec validation; the executable is unavailable here.
