...
be allowed to claim it even if the row is still marked active,
and the claim MUST advance `owner_epoch` so late cleanup from the previous
session is fenced. A row actively owned by another instance MUST retain the
existing `bridge_instance_mismatch` behavior.

#### Scenario: replacement after a clean upstream close

- **WHEN** the previous bridge session closes cleanly and its durable release
  races replacement creation
...
- **WHEN** a follow-up uses `previous_response_id` after the prior bridge socket
  closed cleanly
