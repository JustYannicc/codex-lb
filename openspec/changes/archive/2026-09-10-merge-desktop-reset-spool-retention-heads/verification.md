...
The affected settings/reset/migration run passed124 cases with6 PostgreSQL-only cases skipped locally. Public CLI upgrade/check reports one head, migration policy ok and no schema drift. Ruff, full type checking, architecture checks and67 main specifications pass. Both database variables point to the same disposable database before imports and tests. Hosted PostgreSQL proof remains required for the new committed candidate; b8a33ae47's passing hosted run covers the previous main composition only.
