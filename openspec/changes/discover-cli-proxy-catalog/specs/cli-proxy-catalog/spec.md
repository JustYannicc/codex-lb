...
The system SHALL retain the last successfully validated catalog when discovery fails due to transport, authentication, status, invalid JSON, or invalid catalog structure. Refreshes SHALL have bounded time and body size. Discovery failures SHALL NOT remove native catalog entries or silently substitute models.
