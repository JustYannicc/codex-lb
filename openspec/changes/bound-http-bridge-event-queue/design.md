...
5. **Keep terminal ordering unchanged.** Existing producer call sites retain their event-then-end-marker order. The enqueue helper changes waiting behavior only; it does not reorder, drop while attached, alter spool persistence, or transfer settlement ownership. Queue-full backpressure therefore remains no-drop for an attached consumer, while fail-closed byte-budget revocation has the explicit terminal contract in the change spec.
