...
5. **Keep terminal ordering unchanged.** Existing producer call sites retain their event-then-end-marker order. The enqueue helper changes waiting behavior only; it does not reorder, drop while attached, alter spool persistence, or transfer settlement ownership.
6. **Record why a live queue reached end-of-stream.** A terminal `None` is only a clean prewarm completion when the queue recorded ordered terminal delivery. Budget rejection, ordinary revocation, abort, and discard retain distinct failure outcomes. This state belongs to the finite queue; reader cleanup and delayed attached-consumer delivery remain unchanged.
