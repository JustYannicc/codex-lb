...
1. **Keep the current two-event live queue.** The third unread live event blocks the producer. Terminal event/EOS now use a separate ordered slot, so they no longer justify this capacity. The round-20 producer-task change preserves capacity and the 256 MiB process budget to isolate its performance effect. A different capacity or short delivery-progress timeout remains an explicit owner decision; raising finite capacity alone would not bound a stalled consumer's eventual wait.
...
different transport state. A short delivery-progress bound needs owner
approval of its duration and failure contract. Native transport acceptance
remains separate; neither decision is implied by a passing performance check.
