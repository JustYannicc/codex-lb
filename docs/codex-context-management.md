...
Context tracking is activated by `reasoning.context=all_turns`. A bounded process-local cache avoids repeated writes. After eviction, restart or a move to another replica, an unmarked session checks the durable binding before inference. Existing sessions retain their key fence and participation tracking even when later turns omit the marker. Unbound sessions remain untracked. Explicit notes/history operations always validate their stored binding and current key scope.
